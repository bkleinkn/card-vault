...
- **Parser matches eBay's 2026 results markup**: prices live in `<span class="su-styled-text positive bold large-1 s-card__price">`. Only `positive` (green) spans are genuine SOLD prices — sponsored/active listings mixed into sold results render `primary` (black) and are deliberately excluded, since counting them would re-import exactly the asking-price skew this feature exists to escape. Titles are `class=s-card__title` (attribute **unquoted** in the current markup) and each price is attributed to the nearest preceding title. If comps drop to 0 across the board someday, eBay probably shuffled markup again — fetch one page through the proxy and re-derive; the parser reports zero comps rather than guessing.

**⚠️ The comps must be filtered or they are worse than useless (learned the hard way, 2026-07-21).** The first working version priced a 1976 Topps common — genuinely a $1–$3 card — at **$3,184.97 median, range $457–$97,379**, and the user caught it. Two independent causes, both now fixed and both worth remembering:

1. **eBay pads thin results with a "Results matching fewer words" section.** Only *3* price spans on that page preceded the divider (one real sale, at $2.50); the other 59 were unrelated items eBay tacked on. `parseEbaySoldHtml` now truncates the HTML at that marker before parsing anything.
2. **A non-US proxy exit renders prices in local currency using a bare "$".** Five cards in the first sweep came back scaled ~1000× (max $1,062,903). Fixed at the source by adding `&country_code=us` to `EBAY_PROXY_TEMPLATE` (ScraperAPI geotargeting), plus in-code guards: any price above `NOT_A_CARD_PRICE` ($50k) is dropped, and a letter before the `$` (`C $`, `AU $`, `MX $`) rejects the row.

On top of those, every counted listing must **mention the player's surname and the year in its own title** (`buildRelevance()` → `parseEbaySoldHtml(html, want)`), and the surviving prices are IQR-trimmed (`dropOutliers`) so a complete-set lot can't drag the range. Result on the same two pages: the Cochrane went 60 comps → **1 comp at $2.50**, while a real 1956 Mantle stayed at **53 comps, median $1,200**.

**Thin data still beats the model.** One matching sale now anchors the estimate (±45%, note says "thin data, so treat it as a rough marker") instead of the old ≥3 rule that silently left commons on the AI's inflated guess. ≥3 comps keeps the tighter ±30% band.

**`PRICE_ALGO` (functions/index.js) and the client's copy in app.js must stay in sync.** Cards store `pricesAlgo` alongside `pricesRefreshedAt`; `needsPriceRefresh()` re-offers the Collection sweep for anything below the current version. That is how a pricing fix reaches the ~190 cards already priced by the broken version — bump it whenever the numbers a user already saw would come out different.
...
users/{uid}/cards/{cardId}
  ...
  imageFrontUrl
  imageBackUrl?
  rotation?        // { front?: 0|90|180|270, back?: … } display-only rotation.
                   // Missing/0 ⇒ upright. Storage images are never re-encoded.
  ...
  pricesRefreshedAt? // ISO timestamp of the last price lookup. Absent ⇒ never
                     // priced against real sold listings (i.e. scanned pre-2026-07-21)
  pricesAlgo?      // version of the pricing logic that produced the numbers above.
                   // Below PRICE_ALGO ⇒ the Collection sweep offers the card again
  userNotes?       // free text
...
**Public view-only share link.** From the Collection view ("Share") the owner can create / copy / revoke an unguessable link backed by `shares/{token}`. Anyone with the link opens `#/share/{token}` to see a read-only gallery of the owner's **kept** cards. `getSharedCollection` is a **whitelist** — only `id`, `itemType`, `identified`, `imageFrontUrl`, `imageBackUrl`, `rotation` are returned — so dollar values, comps, notes, and locations never reach the public view, and fields added to cards later can't start leaking by default. The recipient view hides the app's nav chrome (`body.viewing-share`).

The grid shows **fronts**; tapping a tile opens the viewer with **both sides** and Front/Back chips (tiles with a back photo carry a "Front + back" badge, since the back is otherwise invisible to a recipient). `renderSharedView` keeps the payload in `sharedCardsCache` because the tile markup has no back URL to read — the viewer looks the card up by `data-card-id`. This was broken on first release: the back was in the payload but nothing ever rendered it.

**Recipients get search / sort / filter too** (`shareFilters` + `applyShareFilters` + `drawSharedGrid`), a deliberately smaller set than the owner's: no price sorts and no location filter, because neither is in the payload. Two things to know if you touch it:

- **`createdAt` is stripped**, so "newest/oldest added" can't be computed from the data. The server returns cards newest-first and `renderSharedView` stamps `__order` on each one as it caches them; those two sorts are just that index forwards or backwards. Reusing the owner's `sortComparators.newest` here would silently no-op (every `tsMs` is 0).
- **Filter options are built from the cards actually present** — a baseball-only shoebox doesn't offer a Hockey filter, and the Type select and Rookie/HOF chips are omitted entirely when the collection has nothing to distinguish. The shell (head + controls) renders once and only `#share-grid` repaints, so typing in the search box doesn't tear out the input under the cursor.

**Recipients can rotate too, locally.** The viewer's ⟳ button is offered on a share link, but it writes to `localStorage` (`cv-share-rotations`, keyed `token:cardId:side`) instead of Firestore — a recipient has no write access and the owner's card must not change. `viewerState.rotateMode` picks the behaviour: `"save"` (owner → Firestore), `"local"` (share → device only), `"none"` (button hidden). `sharedRotation()` draws the recipient's own adjustment when there is one, else the owner's saved angle. Verified: after a recipient rotates, the owner's doc still has no `rotation` field at all.

**Team / Brand sorts** (`team-az`, `set-az`) exist in both views via the shared `groupSort()` helper: blanks sort **last** (a sealed pack has no team), then year, then player, so each run reads in a sensible order rather than an arbitrary one. In the owner's collection the sort cascades through the Manufacturer→Year→Team grouping as every sort does.
...
**Rotate photos (saved, not re-encoded).** A card photographed sideways is fixed with the **⟳ Rotate** button in the full-screen viewer — one tap per quarter turn, four taps back to upright. The angle is stored on the card as `rotation: { front, back }` (per side) and applied as a CSS class (`rot-90/180/270`) everywhere the photo is drawn: collection thumbnail, detail/review frames, the viewer, and the public share gallery. **The image bytes in Storage are never touched**, so rotating is instant, costs nothing, and is reversible — the trade-off is that a re-identify still sends the AI the original orientation. Because a CSS transform doesn't change the element's layout box, a quarter turn inside the fixed 5/7 frame needs a matching scale: `object-fit: cover` frames (thumbnails, share tiles) scale **up** by 7/5 to keep covering, `object-fit: contain` frames (detail/review, `.photo-frame`) scale **down** by 5/7 to keep fitting, and the viewer computes its own fit in JS since its image is free-floating. Detail/review photos moved into `.photo-frame` for exactly this reason — an untransformed `<img>` would paint over the buttons around it. Recipients of a share link see the corrected orientation but get **no** rotate button (no card to save to); `getSharedCollection` passes `rotation` through as the only non-display addition to its whitelist.
